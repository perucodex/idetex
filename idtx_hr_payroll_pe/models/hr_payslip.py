...
class HrPayslip(models.Model):
    ...
    # El cálculo de la planilla es POR ASISTENCIAS (marcaciones). El modo
    # TAREO (Excel/plantilla) vive en el módulo opcional
    # idtx_hr_payroll_pe_template, que sobreescribe _pe_compute_attendance_data
    # y _get_worked_day_lines cuando la compañía lo activa.
    ...
    def _pe_compute_attendance_data(self):
        ...
        # Las HORAS EXTRA ya no se calculan aquí: las detecta el ruleset de
        # Odoo (hr.attendance.overtime) y se pagan solo las APROBADAS (bloque
        # tras el bucle). El tope/tolerancia de HE se configura en el ruleset.
        ...
        while current <= end:
            ...
            # HORAS EXTRA: ya NO se calculan aquí desde la marcación. Las
            # calcula el ruleset estándar de Odoo (hr.attendance.overtime) y la
            # planilla toma SOLO las horas APROBADAS (ver bloque tras el bucle).

            current += timedelta(days=1)

        # HORAS EXTRA (ruleset de Odoo): solo las HE APROBADAS por día, con el
        # escalonado legal peruano (primeras 2h del día al 25%, resto al 35%).
        # El ruleset (hr.attendance.overtime.ruleset) las detecta desde las
        # marcaciones y RRHH las aprueba (overtime.line.status='approved');
        # mientras no estén aprobadas NO se pagan.
        ...
        ot_by_day = {}
        for l in ot_lines:
            ot_by_day[l.date] = ot_by_day.get(l.date, 0.0) + max(l.duration, 0.0)
        for dur in ot_by_day.values():
            h25 = min(dur, 2.0)   # primeras 2h del día al 25%
            he25 += h25
            he35 += max(dur - 2.0, 0.0)   # resto al 35%
