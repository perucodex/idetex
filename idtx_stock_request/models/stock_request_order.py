...
class StockRequestOrder(models.Model):
    ...
    def default_get(self, fields):
        ...
        if warehouse:
            ...
            res["location_id"] = warehouse._get_stock_request_default_location().id
        return res
    ...
    note = fields.Text(
        string="Notes",
        help="Internal note for the warehouse. It is copied to the generated transfer.",
    )
    request_reference_id = fields.Many2one(
        ...
        readonly=True,
        copy=False,
        help="Reference created on confirmation and shared by all the lines of "
        "the request so that they are grouped in a single transfer.",
    )
    ...
    def onchange_warehouse_id(self):
        if self.warehouse_id:
            ...
            if self.warehouse_id != loc_wh:
                self.location_id = (
                    self.warehouse_id._get_stock_request_default_location()
                )
    ...
    def action_confirm(self):
        if not self.stock_request_ids:
            raise UserError(
                self.env._(
                    "There should be at least one request item for "
    ...
    # ------------------------------------------------------------------
    # Reporte "Requerimiento de productos" (formato papel IDETEX)
    # ------------------------------------------------------------------
    _REPORT_MONTHS = (
    ...
    def _report_date_label(self):
        """Request date as printed on the paper form: 14/Setiembre/2026."""
        self.ensure_one()
        value = fields.Datetime.context_timestamp(
            self, self.create_date or fields.Datetime.now()
        )
        return f"{value.day:02d}/{self._REPORT_MONTHS[value.month - 1]}/{value.year}"

    def _report_area(self):
        """Requesting area: department of the requester when known, else
        the destination location."""
        self.ensure_one()
        user = self.requested_by.sudo()
        department = False
        if "employee_id" in user._fields:  # hr installed
            department = user.employee_id.department_id.name
        return (department or self.location_id.name or "").upper()
    ...
    def _check_before_unlink(self):
        if self.filtered(lambda r: r.state != "draft"):
            raise UserError(self.env._("Only orders on draft state can be unlinked"))
    ...
    def _create_from_product_multiselect(self, products):
        ...
        if products._name not in ("product.product", "product.template"):
            raise ValidationError(
                self.env._("This action only works in the context of products")
            )
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "idtx_stock_request.stock_request_order_action"
        )
        action["views"] = [
            (self.env.ref("idtx_stock_request.stock_request_order_form").id, "form")
        ]
