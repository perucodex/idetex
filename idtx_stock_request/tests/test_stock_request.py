...
class TestStockRequest(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        cls.stock_request_user_group = cls.env.ref(
            "idtx_stock_request.group_stock_request_user"
        )
        ...
        cls.stock_request_manager = new_test_user(
            ...
            groups="idtx_stock_request.group_stock_request_manager",
            company_ids=[(4, cls.main_company.id), (4, cls.company_2.id)],
        )
        cls.product = cls._create_product("SH", "Shoes", False)
        cls.product_company_2 = cls._create_product("SH_2", "Shoes", cls.company_2.id)
        # These tests cover the upstream OCA behaviour (default destination =
        # stock location, procurement through the routes): run them with the
        # IDETEX direct transfer mode disabled.
        (cls.main_company | cls.company_2).write(
            {"stock_request_direct_transfer": False}
        )
