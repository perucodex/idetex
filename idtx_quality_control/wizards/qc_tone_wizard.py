...
class ControlTonoWizard(models.TransientModel):
    _name = "qc.tone.wizard"
    _description = "Wizard Evaluar Tono"

    batch_id = fields.Many2one("mrp.workorder.batch", required=True, ondelete="cascade")
    ...
    def default_get(self, fields_list):
        ...
        line_id = self.env.context.get("default_batch_id")
        if not line_id:
            return res

        line = self.env["mrp.workorder.batch"].browse(line_id)
        if not line.exists():
            return res
        ...
        res.update({
            "batch_id": line.id,
            "cliente": line.qc_customer or "",
            "articulo": (line.qc_article or ""),
            "pedido": line.qc_order_ref or "",
            "partida": line.name or "",
            "hdr": line.qc_route or "",
