...
const DRAFT_STORAGE_KEY = "idtx_quality_control.defect_screen.draft.v4";
...
export class QualityDefectScreen extends Component {
    ...
    async loadApariencias() {
        ...
        this.state.error = "";
        try {
            const apariencias = await this.orm.call("qc.appearance.line", "action_tablet_get_apariencias", ["", 50]);
    ...
    async loadPartidas() {
        ...
        try {
            ...
            const partidas = await this.orm.call("qc.appearance.line", "action_tablet_get_partidas", ["", 50]);
    ...
    async searchApariencias(query) {
        ...
        try {
            const q = (query || "").trim();
            const apariencias = await this.orm.call("qc.appearance.line", "action_tablet_get_apariencias", [q, 50]);
            this.state.apariencias = apariencias || [];
            this._syncSelectedAparienciaData();
            this._saveDraft();
        } catch (error) {
    ...
    async searchPartidas(query) {
        ...
        try {
            ...
            const q = (query || "").trim();
            const partidas = await this.orm.call("qc.appearance.line", "action_tablet_get_partidas", [q, 50]);
            this.state.partidas = partidas || [];
            this._syncSelectedPartidaData();
            this._saveDraft();
        } catch (error) {
    ...
    async confirmFinalize() {
        ...
        try {
            ...
            await this.orm.call("qc.appearance.line", "action_tablet_finalize", [
                Number(this.state.selectedPartidaId),
...
QualityDefectScreen.template = "idtx_quality_control.QualityDefectScreen";
registry.category("actions").add("idtx_quality_control.defect_screen", QualityDefectScreen);