...
const DRAFT_STORAGE_KEY = "idtx_quality_control.dimrev_screen.draft.v2";
...
export class QualityDimrevScreen extends Component {
    ...
    async loadPartidas(query = "") {
        ...
        this.state.error = "";
        try {
            this.state.partidas = await this.orm.call("qc.dimstab.eval", "action_tablet_get_partidas", [query, 100]);
            this._syncSelectedPartidaData();
        } catch (error) {
    ...
    async _loadPartidaEvaluation() {
        ...
        try {
            const payload = await this.orm.call("qc.dimstab.eval", "action_tablet_get_eval_context", [
    ...
    async chooseRepeatEdit() {
        ...
        if (mode === "l1") {
            ...
            try {
                payload = await this.orm.call("qc.dimstab.eval", "action_tablet_get_wash_payload", [
                    Number(this.state.selectedPartidaId),
                    washN,
                    this.state.sampleType,
                ]);
    ...
    async confirmFinalize() {
        ...
        this.state.error = "";
        try {
            const result = await this.orm.call("qc.dimstab.eval", "action_tablet_finalize", [
                Number(this.state.selectedPartidaId),
                this.state.evalMode,
                payload,
                this.state.sampleType,
...
QualityDimrevScreen.template = "idtx_quality_control.QualityDimrevScreen";
registry.category("actions").add("idtx_quality_control.dimrev_screen", QualityDimrevScreen);
