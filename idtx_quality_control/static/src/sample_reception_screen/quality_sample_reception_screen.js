...
export class QualitySampleReceptionScreen extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            const initialPartidaId = Number(this.props.action?.params?.batch_id || 0);
    ...
    async _loadPartidas(query = "") {
        try {
            this.state.partidas = await this.orm.call("qc.sample.reception", "action_tablet_get_partidas", [query, 100]);
    ...
    async _loadHistory() {
        ...
        try {
            const rows = await this.orm.call("qc.sample.reception", "action_tablet_get_recent", [Number(this.state.selectedPartidaId), 12]);
    ...
    async onLookupDni() {
        ...
        try {
            const payload = await this.orm.call("qc.sample.reception", "action_tablet_lookup_dni", [this.state.dni]);
    ...
    async saveReception() {
        ...
        this.state.error = "";
        try {
            await this.orm.call("qc.sample.reception", "action_tablet_register", [
...
QualitySampleReceptionScreen.template = "idtx_quality_control.QualitySampleReceptionScreen";

registry.category("actions").add("idtx_quality_control.sample_reception_screen", QualitySampleReceptionScreen);
