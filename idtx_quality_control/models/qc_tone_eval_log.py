...
class ControlTonoEvalLog(models.Model):
    ...
    cliente = fields.Char(
        string="Cliente",
        related="batch_id.qc_customer",
        readonly=True,
        store=False,
    )
    color = fields.Char(
        string="Color",
        related="batch_id.color_name",
        readonly=True,
        store=False,
    )
    ...
    reprocess_cycle = fields.Char(string="Reprocess", readonly=True)
    ...
    def _get_salesperson_user(self):
        self.ensure_one()
        salesperson = self.batch_id.qc_salesman_id
    ...
    def _send_pending_notification(self):
        self._post_message_from_template(
            "idtx_quality_control.mail_template_tono_eval_pending",
            "_get_pending_notification_emails",
        )

    def _send_client_response_notification(self):
        self._post_message_from_template(
            "idtx_quality_control.mail_template_tono_eval_client_response",
    ...
    def unlink(self):
        ...
        grouped_pairs = [
            (log.eval_group_id.id, log.batch_id.id)
            for log in self
            if log.eval_group_id and log.batch_id
        ]
        ...
        if grouped_pairs:
            line_model = self.env["qc.tone.eval.group.line"]
            log_model = self.env["qc.tone.eval.log"]
            ...
            group_model = self.env["qc.tone.eval.group"]
    ...
    def _get_observacion(self):
        ...
        if self.reprocess_cycle and self.reprocess_cycle.strip() not in ('0', ''):
            partes.append(_('Reproceso: %s') % self.reprocess_cycle)
        return ' — '.join(partes)
    ...
    def action_export_tono_eval_xlsx(self):
        ...
        return {
            ...
            'url': '/idtx_quality_control/tono_eval_xlsx?ids=%s&company_id=%s'
