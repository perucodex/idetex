...
class ControlAparienciaLine(models.Model):
    ...
    apariencia_id = fields.Many2one('qc.appearance', string='Apariencia', required=True)
    ...
    @api.constrains("evaluacion_id", "batch_id", "apariencia_id")
    def _check_evaluacion_matches_roll(self):
        for rec in self:
            if not rec.evaluacion_id:
                continue
            if rec.evaluacion_id.batch_id != rec.batch_id:
    ...
    @api.constrains("batch_id", "apariencia_id", "evaluacion_id", "sample_type", "rollo_num")
    def _check_rollo_unique_per_partida_and_apariencia(self):
        for rec in self:
            if not rec.batch_id or not rec.apariencia_id or not rec.evaluacion_id or rec.rollo_num <= 0:
                continue
    ...
    def create(self, vals_list):
        eval_model = self.env["qc.appearance.eval"]
        for vals in vals_list:
            ...
            if evaluacion_id:
                ...
                if evaluacion.exists() and not batch_id:
                    vals["batch_id"] = evaluacion.batch_id.id

            if not vals.get("evaluacion_id") and batch_id and apariencia_id:
    ...
    def _get_rollo_unique_create_domain(self, batch_id, apariencia_id, rollo_num, sample_type="acabado"):
        return [
            ("batch_id", "=", batch_id),
            ("apariencia_id", "=", apariencia_id),
            ("evaluacion_id.sample_type", "=", sample_type),
    ...
    def _normalize_sample_type(self, sample_type):
        ...
        valid_sample_types = {key for key, _label in self.env["qc.appearance.eval"]._SAMPLE_TYPE_SELECTION}
    ...
    def _sample_type_label(self, sample_type):
        selection = dict(self.env["qc.appearance.eval"]._SAMPLE_TYPE_SELECTION)
        return selection.get(sample_type, sample_type or "")

    @api.model
    def action_tablet_check_rollo_available(self, batch_id, rollo_num, apariencia_id=False, evaluacion_id=False, sample_type=False):
        ...
        if evaluacion_id:
            evaluacion = self.env["qc.appearance.eval"].browse(evaluacion_id)
            ...
            if (
                ...
                or evaluacion.batch_id.id != batch_id
        ...
        exists = self.search_count(
            self._get_rollo_unique_create_domain(batch_id, apariencia_id, rollo_num, sample_type)
        )
        if exists:
            return {
                "ok": False,
    ...
    def action_tablet_get_apariencias(self, query="", limit=20):
        ...
        apariencias = self.env["qc.appearance"].search(domain, order="name asc, id asc", limit=safe_limit)
        return [
            {
                "id": ap.id,
                "name": ap.name or "",
    ...
    def _tablet_roll_summary_by_partida(self, pedido_lines):
        ...
        roll_lines = self.search([
            ("batch_id", "in", pedido_lines.ids),
        ], order="apariencia_id, rollo_num asc, id asc")

        grouped = {}
        for roll_line in roll_lines:
            line_groups = grouped.setdefault(roll_line.batch_id.id, {})
        ...
        for batch_id, apariencia_groups in grouped.items():
            groups = []
            ...
            summary_by_line[batch_id] = groups
    ...
    def action_tablet_get_defectos(self, apariencia_id=None):
        ...
        defectos = self.env["qc.appearance.defect"].search(
    ...
    def action_tablet_finalize(
        ...
        pedido_line = self.env["mrp.workorder.batch"].browse(int(batch_id))
        if not pedido_line.exists():
            raise UserError("La partida seleccionada no existe.")

        apariencia_id = int(apariencia_id or 0)
        apariencia = self.env["qc.appearance"].browse(apariencia_id)
        ...
        evaluacion = self.env["qc.appearance.eval"].browse(int(evaluacion_id or 0))
        if evaluacion and not evaluacion.exists():
            raise UserError("La evaluacion seleccionada no existe.")
        if evaluacion and evaluacion.batch_id != pedido_line:
        ...
        if not evaluacion:
            evaluacion = self.env["qc.appearance.eval"].create({
        ...
        for item in selections or []:
            ...
            defecto = self.env["qc.appearance.defect"].browse(defecto_id)
...
class ControlAparienciaDefectoLine(models.Model):
    ...
    tamano_defecto_ids = fields.One2many('qc.appearance.defect.size', 'defecto_line_id', string='Tamaños Defecto')
...
class ControlAparienciaTamanoDefecto(models.Model):
    _name = "qc.appearance.defect.size"
