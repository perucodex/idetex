...
class MrpWorkorderBatch(models.Model):
    """Partida de producción (idtx_mrp) como sujeto del control de calidad.

    Sustituye a ``control.pedido.line`` (TEXPLUS) de ``idtx_batch_quality``: los
    datos de cabecera que antes venían del SQL Server (cliente, pedido, artículo,
    color, kilos, ciclo de reproceso) se derivan aquí de los rollos de la partida,
    sus órdenes de fabricación y el pedido de venta que las originó.
    """
    ...
    # ------------------------------------------------------------------
    # Adaptador: cabecera de la partida derivada de rollos / OF / pedido
    # ------------------------------------------------------------------
    qc_production_ids = fields.Many2many(
        'mrp.production', string='Órdenes de Fabricación',
        compute='_compute_qc_origin_links')
    qc_product_id = fields.Many2one(
        'product.template', string='Artículo Principal',
        compute='_compute_qc_origin', store=True,
        help='Artículo con más kilos dentro de la partida; las evaluaciones '
             'de laboratorio usan sus estándares (análisis del producto).')
    ...
    qc_partner_id = fields.Many2one(
        ...
        compute='_compute_qc_origin', store=True)
    qc_customer = fields.Char('Cliente', compute='_compute_qc_origin', store=True)
    qc_sale_order_ids = fields.Many2many(
        'sale.order', string='Pedidos de Venta', compute='_compute_qc_origin_links')
    qc_order_ref = fields.Char('Pedido', compute='_compute_qc_origin', store=True)
    qc_salesman_id = fields.Many2one(
        'res.users', string='Vendedor', compute='_compute_qc_origin', store=True)
    qc_client_order_ref = fields.Char(
        'O/C Cliente', compute='_compute_qc_origin', store=True)
    qc_order_note = fields.Text(
        'Observaciones del Pedido', compute='_compute_qc_origin_links')
    qc_route = fields.Char('OF / Ruta', compute='_compute_qc_origin', store=True)
    lab_dev_line_id = fields.Many2one(
        'lab.dev.line', string='Lab Dip', compute='_compute_qc_origin', store=True)
    colorfastness_id = fields.Many2one(
        related='lab_dev_line_id.colorfastness_washing_id',
        string='Solidez al Lavado (Lab Dip)')
    density_stability_twisting_id = fields.Many2one(
    ...
    reprocess_cycle = fields.Char(
        ...
        help='Cantidad de reprocesos de la partida (registros de taller de '
             'idtx_mrp_shop). Un ciclo mayor reabre la evaluación de tono '
             'desde Tacho, como hacía BarCodReo en TEXPLUS.')

    def _qc_rolls(self):
        """Rollos que definen la partida; una partida dividida (sin rollos
        propios) se describe por los rollos de sus sub-partidas."""
        self.ensure_one()
        rolls = self.wo_roll_ids
        if not rolls and self.child_batch_ids:
            rolls = self.child_batch_ids.wo_roll_ids
        return rolls

    @api.depends('wo_roll_ids', 'wo_roll_ids.gross_weight', 'wo_roll_ids.product_id',
                 'child_batch_ids.wo_roll_ids',
    ...
    def _compute_qc_origin(self):
        ...
        for rec in self:
            ...
            orders = productions.sale_order_line_id.order_id
            weight_by_tmpl = {}
            for roll in rolls:
                if roll.product_id:
                    weight_by_tmpl[roll.product_id] = (
                        weight_by_tmpl.get(roll.product_id, 0.0) + (roll.gross_weight or 0.0))
            main = max(weight_by_tmpl, key=weight_by_tmpl.get) if weight_by_tmpl else Template
            partners = orders.partner_id
            rec.qc_product_ids = Template.browse([t.id for t in weight_by_tmpl])
            rec.qc_product_id = main
            rec.qc_article = main.name or ''
            rec.qc_partner_id = partners[:1]
            rec.qc_customer = ', '.join(partners.mapped('name'))
            rec.qc_order_ref = ', '.join(orders.mapped('name'))
            rec.qc_salesman_id = orders[:1].user_id
            rec.qc_client_order_ref = ', '.join(
                ref for ref in orders.mapped('client_order_ref') if ref)
    ...
    @api.depends('wo_roll_ids', 'child_batch_ids.wo_roll_ids',
                 'wo_roll_ids.workorder_id.production_id',
                 'wo_roll_ids.workorder_id.production_id.sale_order_line_id')
    def _compute_qc_origin_links(self):
        """Enlaces NO almacenados a las OF y pedidos de venta de la partida."""
        for rec in self:
            productions = rec._qc_rolls().workorder_id.production_id
            orders = productions.sale_order_line_id.order_id
            rec.qc_production_ids = productions
            rec.qc_sale_order_ids = orders
            rec.qc_order_note = '\n'.join(
                tools.html2plaintext(note) for note in orders.mapped('note') if note)

    @api.depends('wo_roll_ids', 'child_batch_ids.wo_roll_ids')
    def _compute_roll_count(self):
        for rec in self:
            rec.roll_count = len(rec._qc_rolls())

    @api.depends('registry_ids.reprocess_number')
    def _compute_reprocess_cycle(self):
        for rec in self:
            rec.reprocess_cycle = str(rec.reprocess_count or 0)
    ...
    user_id = fields.Many2one(
        ...
        help='Responsable de calidad que firma el resumen de laboratorio.')

    @api.depends('qc_customer')
    def _compute_user_id(self):
        for rec in self:
            if not rec.user_id and self.env.user.has_group('quality.group_quality_manager'):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("report_name", _("New")) == _("New"):
                vals["report_name"] = seq_model.next_by_code("mrp.workorder.batch.qc_report_name") or _("New")
        return super().create(vals_list)

    def _compute_est_revirado_eval(self):
        eval_model = self.env["qc.dimstab.eval"]
        for rec in self:
            eval_rec = eval_model.search([("batch_id", "=", rec.id)], limit=1)
    ...
    def _search_est_revirado_eval_id(self, operator, value):
        ...
        if operator in ("=", "!=") and not value:
            line_ids_with_eval = eval_model.search([]).mapped("batch_id").ids
            if operator == "=":
        ...
        if operator in ("=", "!=", "in", "not in"):
            ...
            line_ids = evals.mapped("batch_id").ids
            if operator in ("=", "in"):
    ...
    def action_evaluar_tono(self):
        ...
        return {
            ...
            "res_model": "qc.tone.wizard",
            "view_mode": "form",
            "target": "new",
            "context": {"default_batch_id": self.id, "tone_mode": "tacho"},
        }
    ...
    def action_evaluar_tono_acabado(self):
        ...
        return {
            ...
            "view_mode": "form",
            "target": "new",
            "context": {"default_batch_id": self.id, "tone_mode": "acabado"},
        }
    ...
    def action_evaluar_tono_secado(self):
        ...
        return {
            ...
            "view_mode": "form",
            "target": "new",
            "context": {"default_batch_id": self.id, "tone_mode": "secado"},
        }
    ...
    def _get_dimrev_highest_wash_eval(self, sample_type=False):
        ...
        return self.env["qc.dimstab.eval"].search(domain, order="wash_number desc, fecha_eval desc, id desc", limit=1)
    ...
    def _get_dimrev_first_eval_for_density_width(self, sample_type=False):
        ...
        if sample_type:
            domain.append(("sample_type", "=", sample_type))
        evals = self.env["qc.dimstab.eval"].search(domain, order="fecha_eval asc, id asc")
        if not evals:
            return evals

        # Prefer the latest first-wash that ended in pass (including criteria-forced pass).
        latest_pass = self.env["qc.dimstab.eval"].search(
            domain + [("state", "=", "pass")],
    ...
    def _get_solidez_latest_eval(self):
        self.ensure_one()
        return self.env["qc.wash.fastness.eval"].search([
            ("batch_id", "=", self.id),
    ...
    def _laboratorio_summary_report_payload(self):
        ...
        analysis = self.qc_product_id.analysis_id
        ...
        if not labdev and solidez:
            labdev = solidez.batch_id.lab_dev_line_id
        if not labdev:
    ...
    def action_print_laboratorio_summary_report(self):
        ...
        template = self.env.ref("idtx_quality_control.mail_template_laboratorio_summary_front", raise_if_not_found=False)
        recipient = self._get_mail_recipient_partner()
        base_url = self.env["ir.config_parameter"].sudo().get_param("web.base.url")

        ctx = {
            "default_model": "mrp.workorder.batch",
            ...
            "action_button_url": f"{base_url}/web#id={self.id}&model=mrp.workorder.batch&view_type=form",
        }
        if template:
    ...
    def get_laboratorio_public_url(self):
        ...
        return f"{base_url}/qc/lab/{self.id}?{query}"
    ...
    def _get_mail_recipient_partner(self):
        """Destinatario del resumen de laboratorio: el cliente del pedido de
        venta de la partida; si no tiene correo, otro contacto con ese nombre.
        Devuelve siempre un recordset (vacío si no hay candidato) para que la
        plantilla de correo pueda evaluar ``.lang`` sin fallar."""
        self.ensure_one()
        partner_model = self.env["res.partner"]
        if self.qc_partner_id and self.qc_partner_id.email:
            return self.qc_partner_id

        customer_name = (self.qc_customer or "").strip()
    ...
    def action_apariencia(self):
        ...
        return {
            ...
            "tag": "idtx_quality_control.defect_screen",
        }
    ...
    def action_sample_reception_screen(self):
        ...
        return {
            ...
            "tag": "idtx_quality_control.sample_reception_screen",
            "params": {
                "batch_id": self.id,
            },
        }
    ...
    @staticmethod
    def _tono_reprocess_cycle_rank(value):
        try:
    ...
    def _get_tono_tablet_state(self):
        ...
        current_rank = self._tono_reprocess_cycle_rank(self.reprocess_cycle)

        same_cycle_logs = all_logs.filtered(
            lambda l: self._tono_reprocess_cycle_rank(l.reprocess_cycle) == current_rank
        )
        ...
        max_tacho_rank = max(
            (self._tono_reprocess_cycle_rank(log.reprocess_cycle) for log in tacho_final_logs),
            default=0.0,
        )
        ...
        if same_cycle_logs:
            ...
            logs = self.env["qc.tone.eval.log"]
        else:
            logs = all_logs
    ...
    def _build_tono_partida_payload(self):
        ...
        return {
            "id": self.id,
            "label": f"{self.name or '-'} | {self.qc_customer or '-'}",
            "batch": self.name or "",
            "customer": self.qc_customer or "",
            "article": self.qc_article or "",
            "color_name": self.color_name or "",
            "color_code": self.color_code or "",
            "kilograms": self.kilograms or 0.0,
            "pedido": self.qc_order_ref or "",
            "hdr": self.qc_route or "",
            "mode": mode,
    ...
    def action_tablet_get_partidas_tono(self, query="", limit=20):
        ...
        lines = self.search(domain, order="name desc, id desc", limit=safe_limit)

        tacho_done_line_ids = set(
            self.env["qc.tone.eval.log"].search([
                ("batch_id", "in", lines.ids),
                ("tono", "=", "tacho"),
        ...
        grouped = {}
        for line in lines:
            batch_key = (line.name or "").strip()
            is_rect = line.qc_product_id.analysis_id.weave_type == "rect"
        ...
        for group_key, group_lines in grouped.items():
            ...
            if len(group_lines) > 1:
                product_names = [
                    (line.qc_product_id.display_name or line.qc_article or "")
                    for line in group_lines
                    if (line.qc_product_id.display_name or line.qc_article)
                ]
                unique_names = list(dict.fromkeys(product_names))
                article_value = "Agrupado: %s" % ", ".join(unique_names) if unique_names else "Agrupado"
            elif first.qc_product_id.analysis_id.weave_type == "rect":
                article_value = first.qc_product_id.display_name or first.qc_article or ""
            else:
                article_value = first.qc_article or ""

            payloads.append({
                "id": group_key,
                "line_ids": group_lines.ids,
                "line_count": len(group_lines),
                "label": f"{first.name or '-'} | {first.qc_customer or '-'}",
                "batch": first.name or "",
                "customer": first.qc_customer or "",
                "article": article_value,
                "color_name": first.color_name or "",
                "color_code": first.color_code or "",
                "kilograms": float(sum(group_lines.mapped("kilograms")) or 0.0),
                "pedido": first.qc_order_ref or "",
                "hdr": first.qc_route or "",
                "mode": state.get("mode"),
                "mode_label": mode_label,
                "available_modes": available_modes,
                "can_evaluate": bool(state.get("can_evaluate")),
    ...
    @api.model
    def action_tablet_get_tono_context(self, batch_ids):
        line_ids = []
        if isinstance(batch_ids, int):
            line_ids = [batch_ids]
        elif isinstance(batch_ids, list):
            line_ids = [int(x) for x in batch_ids if x]
        ...
        ultimo_tacho = self.env["qc.tone.eval.log"].search(
            [("batch_id", "in", lines.ids), ("tono", "=", "tacho")],
            order="fecha_eval desc, id desc",
            limit=1,
        )
    ...
    def action_tablet_submit_tono(
        ...
        if isinstance(batch_id, int):
            ...
            line_ids = [int(x) for x in batch_id if x]
        ...
        created_count = 0
        group = self.env["qc.tone.eval.group"].sudo().create({
            "tono": selected_mode,
            "resultado": decision,
        ...
        for line in expanded_lines:
            ...
            if not line_state.get("can_evaluate") or selected_mode not in line_modes:
                raise UserError(
                    ...
                    % (line.name or line.display_name, selected_mode)
                )
            ...
            if selected_mode in ("secado", "acabado"):
                ultimo_tacho = self.env["qc.tone.eval.log"].search(
                    [
                        ("batch_id", "=", line.id),
                        ("tono", "=", "tacho"),
                        ("reprocess_cycle", "=", line.reprocess_cycle or ""),
                    ],
                    order="fecha_eval desc, id desc",
                    limit=1,
                )
            ...
            self.env["qc.tone.eval.group.line"].sudo().create({
                "group_id": group.id,
                "batch_id": line.id,
            })
            created_count += 1
