...
class ControlTonoEvalGroup(models.Model):
    ...
    line_ids = fields.One2many("control.tono.eval.group.line", "group_id", string="Partidas")
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("name", "New") == "New":
                vals["name"] = seq.next_by_code("control.tono.eval.group") or "New"
        return super().create(vals_list)

    def unlink(self):
        # Remove linked logs with bypass context so grouped deletion works end-to-end.
        self.env["control.tono.eval.log"].with_context(allow_group_eval_log_unlink=True).search(
...
class ControlTonoEvalGroupLine(models.Model):
    _name = "control.tono.eval.group.line"
    ...
    pedido_line_id = fields.Many2one("control.pedido.line", string="Partida", required=True, index=True)

    customer = fields.Char(string="Cliente", related="pedido_line_id.pedido_id.customer", store=False, readonly=True)
    article = fields.Char(string="Articulo", related="pedido_line_id.description", store=False, readonly=True)
    kilograms = fields.Float(string="Kilos", related="pedido_line_id.kilograms", store=False, readonly=True)
    ...
    def unlink(self):
        ...
        for line in self:
            ...
            label = line.pedido_line_id.batch or line.pedido_line_id.display_name or str(line.pedido_line_id.id)
