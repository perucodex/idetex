...
class LaboratorioPublicController(http.Controller):
    ...
    @http.route('/lab/resultado/<int:line_id>', type='http', auth='public')
    def laboratorio_resultado_publico(self, line_id, access_token=None, **kwargs):
        line = request.env['control.pedido.line'].sudo().browse(line_id)
        if not line.exists():
        ...
        return request.render('idtx_batch_quality.laboratorio_resultado_publico_page', values)
